...
- **Dashboard Top 20 SKUs (2 side-by-side cards)** — new "Top 20 SKUs by Turnover" (left) + "Top 20 SKUs by Units Sold / Bestsellers" (right). Respects current FiltersBar (date range / marketplace / sku). Backend: `GET /api/dashboard/top-skus?sort_by=units|revenue&limit=20` (added `sort_by` param, pattern-validated).
- Files touched: `/app/backend/routes/dashboard.py` (summary, profit_loss, top_skus + sort_by), `/app/backend/routes/library.py` (library_skus, loss_makers), `/app/frontend/src/pages/Dashboard.jsx`, `/app/frontend/src/pages/Settings.jsx`, `/app/frontend/src/lib/i18n.jsx`.
